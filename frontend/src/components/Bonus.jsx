...
function Bonus() {
  ...
  const fetchBonus = useCallback(
    async (offset, limit, search, employee, amountFrom, amountTo) => {
      try {
        ...
        setRows(
          res.data.bonus.map((el) => {
            return {
              ...el,
              date: `${new Date(el.date)
                .getDate()
                .toString()
                .padStart(2, 0)}/${(new Date(el.date).getMonth() + 1)
                .toString()
                .padStart(2, 0)}/${new Date(el.date).getFullYear()}`,
            };
          })
        );
        setState((prevState) => {
  ...
  const createBonus = useCallback(async () => {
    try {
      const res = await axios.post(
        `${process.env.REACT_APP_API}/api/bonus`,
        {
  ...
  const deleteBonus = useCallback(async () => {
    try {
      const res = await axios.delete(
        `${process.env.REACT_APP_API}/api/bonus/${state.selectedRow
        ...
        >
          <Grid item xs={5}>
            <Card>
              <CardHeader
                title="Create Bonus"
                subheader="Grant a bonus to an employee."
              />
              <CardContent>
                <TextField
                  fullWidth
          ...
          <Grid item xs={5}>
            <Card>
              ...
              <CardActions>
                ...
                <Button variant="contained" color="error" onClick={deleteBonus}>
                  Delete
                </Button>
              </CardActions>
            </Card>
