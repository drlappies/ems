...
function Employee() {
    ...
    async (
      ...
      try {
        const res = await axios.get(
          `${process.env.REACT_APP_API}/api/employee`,
          {
            params: {
              offset: offset,
              limit: limit,
        ...
        setRows(
          res.data.employee.map((el) => {
            return {
              ...el,
              onboard_date: `${new Date(el.onboard_date)
                .getDate()
                .toString()
                .padStart(2, 0)}/${(new Date(el.onboard_date).getMonth() + 1)
                .toString()
                .padStart(2, 0)}/${new Date(el.onboard_date).getFullYear()}`,
  ...
  const createEmployee = useCallback(async () => {
    try {
      const body = {
        ...
        onboard_date: state.createOnboardDate
          ? state.createOnboardDate.format("YYYY-MM-DD")
          : null,
        salary_monthly: state.createSalary,
        start_hour: state.createStartHour
          ? state.createStartHour.format("HH:mm:ss")
          : null,
        end_hour: state.createEndHour
          ? state.createEndHour.format("HH:mm:ss")
          : null,
        ...
        ot_hourly_salary:
          state.createHasOTpay === "entitled" ? state.createOTpay : null,
    ...
    fetchEmployee,
    state.createAddress,
    state.createContactNum,
    state.createContactPerson,
    state.createDept,
    state.createEndHour,
    state.createFirstname,
    state.createHasOTpay,
    state.createLastname,
    state.createNum,
    state.createOTpay,
    state.createOnboardDate,
    state.createPassword,
    state.createPos,
    state.createRole,
    state.createSalary,
    state.createStartHour,
  ...
  const deleteEmployee = useCallback(async () => {
    try {
      const res = await axios.delete(
        `${process.env.REACT_APP_API}/api/employee/${state.selectedRow
  ...
  const changePageSize = (limit) => {
    setState((prevState) => {
      return { ...prevState, limit: limit };
    });
  };
  const changePage = (offset) => {
    setState((prevState) => {
      return { ...prevState, offset: offset };
    });
  };
    ...
    if (state.selectedRow.length < 2) {
      ...
      setState((prevState) => {
        return {
          ...
          createHasOTpay: res.data.employee.ot_pay_entitled
            ? "entitled"
            : "unentitled",
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={2}>
                    <TextField
                      ...
                      select
                      size="small"
                      label="OT Compensation"
                      name="hasOTpay"
                      value={state.hasOTpay}
                      onChange={handleChange}
                    >
                      <MenuItem value="any">Any</MenuItem>
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={6}>
                    <TextField
                      ...
                      value={state.createContactPerson}
                  ...
                  <Grid item xs={6}>
                    <TextField
                      ...
                      value={state.createContactNum}
                      name="createContactNum"
                      onChange={handleChange}
                    />
                  </Grid>
                  <LocalizationProvider dateAdapter={DateAdapter}>
                    ...
                    <Grid item xs={6}>
                      <TimePicker
                        ...
                        onChange={(newValue) => {
                          setState((prevState) => {
                            return { ...prevState, createStartHour: newValue };
                          });
                        }}
                        renderInput={(params) => (
                          <TextField fullWidth size="small" {...params} />
                    ...
                    <Grid item xs={6}>
                      <TimePicker
                        ...
                        onChange={(newValue) => {
                          setState((prevState) => {
                            return { ...prevState, createEndHour: newValue };
                          });
                        }}
                        renderInput={(params) => (
                          <TextField fullWidth size="small" {...params} />
                  ...
                  <Grid item xs={6}>
                    <TextField
                      ...
                      type="number"
                      fullWidth
                      value={state.createOTpay}
                      name="createOTpay"
                      label="OT Hourly Compensation"
                      onChange={handleChange}
                      disabled={state.createHasOTpay === "unentitled"}
                    />
                  </Grid>
                  <Grid item xs={6}>
                    <TextField
